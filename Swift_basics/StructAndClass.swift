...
class StructAndClass: UIViewController {
    ...
    /**结构体和枚举是值类型
     ...
     在Swift中，所有的基本类型：整数（Integer）、浮点数（floating-point）、布尔值（Booleans）、字符串（string)、数组（array）和字典（dictionaries），都是值类型，并且都是以结构体的形式在后台所实现。
     在 Swift 中，所有的结构体和枚举都是值类型。这意味着它们的实例，以及实例中所包含的任何值类型属性，在代码中传递的时候都会被复制。
     */
        //例如:
        var cinema = vga
        //因为Resolution是一个结构体,所以会先创建一个现有实例的副本，然后将副本赋值给 cinema,而不是vga本身.尽管vga和cinema有着相同的宽和高的属性, 但是在幕后,它们是两个安全不同的实例.
        cinema.width = 2048 //
...
//注意:需要注意的是tenEighty和alsoTenEighty被声明为常量而不是变量。然而你依然可以改变tenEighty.frameRate和alsoTenEighty.frameRate,因为这两个常量本身不会改变。它们并不存储这个VideoMode实例，在底层仅仅是对VideoMode实例的引用。所以，改变的是被引用的基础VideoMode的frameRate参数，而不改变常量的值。
        ...
        //MARK: -- 指针
        //C,C++,OC,使用指针来引用内存中的地址,一个Swift常量或者变量引用一个引用类型的实例与C语言的指针类似,不同的是,并不直接指向内存中的某个地址,而且也不要求你使用* 来表明你在创建一个引用.Swift中这些引用与其他的常量或者变量的定义方式相同
